...
from ..tools import *
def ailft(out_file,out_path='./',nanite=None):
    if nanite == None:
    ...
    while "LOEWDIN ORBITAL-COMPOSITIONS" not in fi.readline():
        continue
    for i in range(3):
        line=fi.readline()
    #while '----------------' not in line:
    ...
    nanite.mos=mos
    nanite.atoms=atoms
    while "The ligand field one electron eigenfunctions:" not in fi.readline():
        continue
    lf_mos=[]
    fi.readline()
    d_orbs=fi.readline().split()[-5:]
    for i in range(5):
        split=fi.readline().split()
        lf_mos.append(MO(split[0],0.0,split[2]))
        for i in range(5):
            if float(split[3+i]) < 0:
                contri=-float(split[3+i])
            else:
                contri=float(split[3+i])
            lf_mos[-1].c.update({d_orbs[i]:contri*100})
        lf_mos[-1].find_major_contributors()
    nanite.lf_mos=lf_mos
    return nanite